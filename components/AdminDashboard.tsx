...
const AdminDashboard: React.FC = () => {
  ...
  const handleDeleteSession = async (e: React.MouseEvent, sessionId: string) => {
      ...
      if (window.confirm("Are you sure you want to delete this session? This action is irreversible.")) {
  ...
  const handleDeleteTemplate = async (e: React.MouseEvent, template: Template) => {
      ...
      if (template.active) {
          if (window.confirm("Deactivate this template? It will be hidden from the selection screen.")) {
  ...
  const handleRunCleanup = async () => {
      if (window.confirm("Run retention cleanup manually? This will delete files for sessions older than 24 hours.")) {
          setIsLoading(true);
          try {
            ...
            <nav className="flex-1 flex flex-col gap-2 px-6">
                {[
                    ...
                    <button
                        ...
                        className={`group flex items-center gap-4 px-4 py-3 rounded-sm transition-all text-left ${
                            activeTab === item.id 
                                    ...
                                    transactions.map(tx => (
                                        <tr key={tx.id} className="hover:bg-white/[0.02] transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center px-2 py-1 rounded-sm text-[10px] uppercase font-bold tracking-wide ${
